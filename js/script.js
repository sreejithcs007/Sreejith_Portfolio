...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function renderBootUI() {
    if (bootProgress) {
      bootProgress.style.width = `${bootProgressValue}%`;
    }

    if (bootPercent) {
      bootPercent.textContent = `${Math.floor(bootProgressValue)}%`;
    }

    if (bootStatus) {
      bootStatus.textContent = bootStages[bootStageIndex];
    }
  }
  ...
  const bootTimer = setInterval(() => {
    if (bootProgressValue < 70) {
      bootProgressValue += Math.random() * 1.5 + 0.4;
    } else if (bootProgressValue < 90) {
      bootProgressValue += Math.random() * 0.8 + 0.2;
    } else {
      bootProgressValue += Math.random() * 0.3 + 0.08;
    }

    if (bootProgressValue >= 25 && bootStageIndex === 0) {
      bootStageIndex = 1;
    }

    if (bootProgressValue >= 55 && bootStageIndex === 1) {
      bootStageIndex = 2;
    }

    if (bootProgressValue >= 85 && bootStageIndex === 2) {
      bootStageIndex = 3;
    }
    ...
    if (bootProgressValue >= 100) {
      ...
      setTimeout(() => {
        ...
        if (appWrapper) {
          ...
          appWrapper.classList.add('app-show');
        }
...
// Keep your second DOMContentLoaded block for the floating stack cards unchanged.